    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-gray-100 flex items-center justify-center text-xl font-bold text-gray-500 flex-shrink-0">
            {candidate.full_name.charAt(0).toUpperCase()}
          </div>
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl font-bold text-[#0F1B2D]">{candidate.full_name}</h1>
              <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${statusConfig.color}`}>
                {statusConfig.label}
              </span>
            </div>
            <p className="text-gray-500 text-sm mt-0.5">{candidate.position_applied}</p>
      ...
      {totalReferees > 0 && (
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5 mb-6 flex items-center gap-6">
          <div>
            <p className="text-xs text-gray-400 mb-1">References Received</p>
            <p className="text-2xl font-bold text-[#0F1B2D]">{completedReferees}<span className="text-sm text-gray-400 font-normal">/{totalReferees}</span></p>
          </div>
          <div className="flex-1">
            <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden">
              <div
                className="h-full bg-[#0A6E6E] rounded-full transition-all"
      ...
      {fraudSignals && fraudSignals.length > 0 && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-5 mb-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold text-red-600">Fraud Alerts ({fraudSignals.length})</h2>
          </div>
          <div className="space-y-2">
            {fraudSignals.map((signal) => (
              <div key={signal.id} className="flex items-center justify-between px-4 py-3 rounded-lg bg-white border border-red-100">
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-[#0F1B2D]">Referees ({referees?.length ?? 0})</h2>
        </div>
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="text-sm font-semibold text-[#0F1B2D]">{referee.full_name}</p>
                        {referee.job_title && (
                          <span className="text-xs text-gray-400">&middot; {referee.job_title}</span>
                        )}
                      </div>
                  ...
                  {referee.status === "completed" && refereeResponses.length > 0 && (
                    <div className="mt-4 space-y-4 pt-4 border-t border-gray-100">
                      {refereeResponses.map((response) => (
                        <div key={response.id}>
                          <p className="text-xs font-medium text-gray-400 mb-1.5">{response.question_text}</p>
                          {response.answer_rating ? (
                            <div className="flex items-center gap-0.5">
                              {[1, 2, 3, 4, 5].map((star) => (
                                <span key={star} className={`text-base leading-none ${star <= response.answer_rating ? "text-[#0A6E6E]" : "text-gray-200"}`}>
                                  ★
                                </span>
                              ))}
                              <span className="text-xs text-gray-400 ml-2">{response.answer_rating}/5</span>
                            </div>
                          ) : (
                            <p className="text-sm text-gray-600 leading-relaxed">{response.answer_text || "—"}</p>
                          )}
                        </div>
                      ))}
                      {refereeResponses.some(r => r.source === "voice_transcript") && (
                        <span className="inline-flex items-center text-xs px-2.5 py-1 rounded-full bg-teal-50 text-teal-700 border border-teal-100">
                          Completed via Voice
                        </span>
                      )}
                    </div>
